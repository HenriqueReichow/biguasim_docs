...
.. note:: 
   The steps in this page are only necessary if you want to change BiguaSim's C++ code or if 
   you want to use Unreal Engine to develop new worlds. It is not necessary for making changes 
   to the Python API or for using BiguaSim as a user.
...
Developing in BiguaSim requires the following additional dependencies:
...
.. note::
   BiguaSim 2.0 requires Numpy version 2. This is a breaking change from BiguaSim 1.0, which used 
   numpy version 1. Please ensure you are using the version of numpy appropriate for the version of
   BiguaSim you are using.
...
==========================
To open the BiguaSim project in the Unreal Editor, find the ``holodeck.uproject`` file in the 
``biguasim/engine`` directory. Double-click this file, and choose "5.3" if an engine version dialog
...
Setting up VSCode 
=================
If you would like to use VSCode instead of Visual Studio for BiguaSim, you can do the following:
...
If you want to use Visual Studio to compile BiguaSim, you will need to generate the .sln file 
...
.. code:: Python
   ...
   with biguasim.make(scenario_cfg=config, start_world=False) as env:
      for _ in range(1000):
...
This log will be saved in biguasim/engine/Saved/Logs if you want to see it later. To view the logs 
