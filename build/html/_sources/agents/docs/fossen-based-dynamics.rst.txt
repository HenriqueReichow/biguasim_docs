...
of Fossen's implementations into BiguaSim to enable accurate vehicle dynamics simulations.

Using Fossen-based dynamics in BiguaSim is an extension of the Custom Dynamics control scheme. To use the
built-in Fossen dynamics, there are just a few steps to configure these improved underwater vehicle dynamics. 
Here is a summary of the steps to set up Fossen dynamics in BiguaSim with
...
- Otter (``Otter: "otter"``)
    ...
    Control Modes:
        ...
        - headingAutopilot: Not verfied yet by BiguaSim developers. Likely works but with the NED reference frame instead of NWU.
...
.. note::
    Mass and other default vehicle parameters set in the BiguaSim engine are ignored when using 
...
an example of using Fossen dynamics in BiguaSim. The full code can be found in `client/example.py`.

Configuration
-------------
Setting up a BiguaSim simulation that uses Fossen dynamics begins with the scenario configuration.
The scenario configuration follows the standard BiguaSim format, with a few key differences for Fossen dynamics:
...
Simulation Setup
----------------
The setup for the simulation is similar to other BiguaSim :ref:`examples`. 
...
.. code-block:: python
    ...
    accel = np.array(np.zeros(6), float)  # Initialize BiguaSim acceleration input
...
The `FossenInterface.update` function takes in the state returned from BiguaSim and parses the data 
from the Dynamics Sensor. Given the state of the vehicle and control surface inputs, it calculates 
an output of accelerations in the BiguaSim frame (NWU).
...
.. code-block:: python
    ...
    for i in range(1500):
        ...
        accel = fossen_interface.update('auv0', state)  # Calculate accelerations to be applied to BiguaSim agent
...
agents in BiguaSim. The full code can be found in `client/example.py`.
...
.. code-block:: python
    ...
    for i in range(1500):
        ...
        for agent in fossen_agents:
            accel = fossen_interface.update(agent, states) #Calculate accelerations to be applied to BiguaSim agent
            env.act(agent, accel)
