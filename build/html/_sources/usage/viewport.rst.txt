...
BiguaSim simulations are run in a pop-up window known as the "viewport". 
...
(:meth:`~biguasim.environments.BiguaSimEnvironment.should_render_viewport`) 
method on a :class:`~biguasim.environments.BiguaSimEnvironment`. This can be
...
In :func:`biguasim.make`, set ``show_viewport`` to ``False``. 
...
.. note::
   ...
   :class:`~biguasim.sensors.ViewportCapture`) when running headless,
   simply set 
   (:meth:`~biguasim.environments.BiguaSimEnvironment.should_render_viewport`)
   to ``True``
